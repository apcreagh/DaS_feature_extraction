function [hd D, QueryDistances] = HausdorffDist(P,Q,lmf,dv)
...
dists = sqrt(sum((cP - cQ).^2,2)); %Eucledian distance
...
try
QueryDistances(:,1)=min(D,[],1);
QueryDistances(:,2)=min(D,[],2);
catch
    QueryDistances=NaN;
end 
...
%     subplot(1,2,1)
    ...
    % -- plot data --
    if sP(2) == 2
        h(1) = plot(P(:,1),P(:,2),'bo','markersize',3,'linewidth',3);
        h(2) = plot(Q(:,1),Q(:,2),'k:','markersize',3,'linewidth',2.5);
       h(1) = plot(P(:,1),P(:,2),'bo','markersize',3,'linewidth',3);
        ...
        h(3) = plot(xyf(:,1),xyf(:,2),'-ko','markersize',12,'linewidth',2);
        uistack(fliplr(h),'top')
%         xlabel('Dim 1'); ylabel('Dim 2');
        set(gca, 'XTick',[],'YTick',[])
        ymin=300;  ymax=2500; xmin=-200;  xmax=1600; 
%         axis([xmin xmax ymin ymax]);
         axis off;
        title({['Hausdorff Distance = ' num2str(m)]; ''; ''})
         legend(h,{'Drawn Shape','Reference Shape','Hausdorff Dist'},'location','bestoutside')
        
    elseif sP(2) == 1   
        ofst = hd/2;    % plotting offset
        h(1) = plot(P(:,1),ones(1,sP(1)),'ro','markersize',3,'linewidth',3);
        h(2) = plot(Q(:,1),ones(1,sQ(1))+ofst,'k:','markersize',3,'linewidth',2.5);
        % draw all minimum distances from set P
        Xp = [P(1:sP(1)) Q(ixp)];
        ...
        uistack(fliplr(h),'top')
        ylabel('visualization offset');
        set(gca, 'XTick',[],'YTick',[])
%         axis ([-100 2000 -200 2000])%         axis([xmin xmax ymin ymax]);
        title(['Hausdorff Distance = ' num2str(m)])
        legend(h,{'Drawn Coordinates','Reference Coordinates','Hausdorff Dist'},'location','best')
        
    elseif sP(2) == 3
        h(1) = plot3(P(:,1),P(:,2),P(:,3),'bx','markersize',3,'linewidth',3);
        h(2) = plot3(Q(:,1),Q(:,2),Q(:,3),'bo','markersize',3,'linewidth',2.5);
        h(1) = plot3(P(:,1),P(:,2),P(:,3),'bx','markersize',3,'linewidth',3);
        ...
        % denote the hausdorff distance
        h(3) = plot3(xyf(:,1),xyf(:,2),xyf(:,3),'-ko','markersize',12,'linewidth',2);
        uistack(fliplr(h),'top')
        xlabel('Dim 1'); ylabel('Dim 2'); zlabel('Dim 3');
        title(['Hausdorff Distance = ' num2str(m)])
        legend(h,{'Drawn Coordinates','Reference Shape','Hausdorff Dist'},'location','best')
...
%     xlabel('ordered points in Q (o) - Drawn Shape')
%     ylabel('ordered points in P (x) - Ref. Shape')
%     title({'Distance (color) between points in Drawn and Reference Shape';...
